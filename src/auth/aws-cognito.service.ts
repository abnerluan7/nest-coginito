...
import { AuthConfirmUserDto } from './dtos/auth-confirm-user-user.dto';
...
export class AwsCognitoService {
  ...
  async confirmUser(authConfirmPasswordUserDto: AuthConfirmUserDto) {
    const { email, confirmationCode } = authConfirmPasswordUserDto;
    ...
    return new Promise((resolve, reject) => {
      userCognito.confirmRegistration(confirmationCode, true, (callback) => {
        resolve({ status: callback });
      });
    });
  }
}
